...
class DepositAdapter( val item: List<DepositObj.DataListRes>) : RecyclerView.Adapter<DepositAdapter.ViewHolder>(){
    ...
    class ViewHolder(var binding:ItemListFinanceDepositPaymentBinding) : RecyclerView.ViewHolder(binding.root) {
        fun bind(data: DepositObj.DataListRes){
            binding.apply {
                Glide.with(imgCardImage).load(data.img_url).fitCenter().into(imgCardImage)
                tvCardName.text = data.tittle
