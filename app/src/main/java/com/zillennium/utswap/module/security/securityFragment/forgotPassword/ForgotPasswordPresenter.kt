...
class ForgotPasswordPresenter : BaseMvpPresenterImpl<ForgotPasswordView.View>(),
    ...
    override fun forgotPassword(body: User.ForgotPasswordObject, context: Context) {
        subscription?.unsubscribe()
        subscription = ApiUserImp().resetPassword(body,context).subscribe({
            if(it.status == 1)
            {
                mView?.onForgotLoginPasswordSuccess(it)
            }else{
                mView?.onForgotLoginPasswordFail(it)
            }
        },{
