...
//
    ...
    override fun projectInfoViewSuccess(data: ProjectInfoDetail.ProjectInfoDetailData) {
        binding.apply {
            /* Image Slider with View Pager and TabLayout*/
            imagesSlider = arrayListOf(data.images.toString())

            val projectInfoSlideImage = arrayListOf<ProjectInfoDetail.ProjectInfoDetailData>()

            println("=============" + data)

            println("======imageslider=====" + imagesSlider)
            for (i in imagesSlider.indices) {
                val projectImage = ProjectInfoDetail.ProjectInfoDetailData(
                    imagesSlider[i],
                )
                projectInfoSlideImage.add(projectImage)
            }
            val adapter = ProjectViewPagerAdapter(projectInfoSlideImage)
            imageSlideViewPager.adapter = adapter
            TabLayoutMediator(tabLayoutDot, imageSlideViewPager) { tab, position ->
            }.attach()
