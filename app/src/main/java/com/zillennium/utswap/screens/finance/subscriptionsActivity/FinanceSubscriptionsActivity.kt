...
//                    FinanceSubscriptionDateRangeBottomSheet().setOnCallBack(callBack)
    ...
    private fun filter() {
        ...
        if (!filter.isNullOrEmpty()) {
            binding.txtTitleFilter.text = filter.toString()

            if (SettingVariable.finance_subscription_filter.value.toString() == "All Projects") {
                if (dateStart.isNullOrEmpty() && dateEnd.isNullOrEmpty()) {
                    financeSubscriptionsAdapter?.filterList(list)
                } else if (!dateStart.isNullOrEmpty() || !dateEnd.isNullOrEmpty()){
                    financeSubscriptionsAdapter?.filterList(list)
                }
            } else {
                if (dateStart.isNullOrEmpty() && dateEnd.isNullOrEmpty()) {
